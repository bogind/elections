...
//     return result;
//     }
//partyConnections=[{party1:"מרצ",party2:"אמת"},{party1:"פה",party2:"כן"},{party1:"מחל",party2:"ט"},{party1:"ג",party2:"שס"}]
nationalResults = []
Promise.all([
    ...
    for (var i = 0; i < responseList.length; i++) {
        ...
        //let myParty = new party(responseList[i][0],responseList[i][1])
        tempParties.push(new party(responseList[i][0],responseList[i][1]))
        //tempParties.push({myParty}) 
...
function calcMandates (){
    ...
    for (var i = 0; i < _parties.length; i++) {
        ...
        if (!_parties[i].connected && _parties[i].aboveBlockPercent)
            partiesForSpare.push(_parties[i]);
    }
